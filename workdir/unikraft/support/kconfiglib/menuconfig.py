...
def _main():
    # print("222 MQY menu tree: " + str(_shown_nodes(standard_kconfig(__doc__))))
    # print("MQY_DOC " + standard_kconfig(__doc__))
    # menuconfig(standard_kconfig(__doc__))
    start_flask(standard_kconfig(__doc__))


def start_flask(kconf):
    global _kconf
    ...
    _cur_menu = _kconf.top_node # move to top menu first
    get_now_info()
    print_menu_info("TOP TEST3333")
    print("MQY kconf " + str(kconf))

    print("MQY menu tree: " + str(_shown_nodes(kconf.top_node)))
    ...
    get_now_info() # refresh menu into _shown
    go_sub(3) # go sub menu and refresh _shown
    set_a_value(16, 0) # set value on a index 
    go_parent() # go parent menu and refresh _shown
    quit_and_save() # quit and save configure to .config
 


# convert a menu to a dict
def _info_dict(node):
    # Returns information about the menu node 'node' as a dictionary.
    ...
    if isinstance(node.item, Symbol):
        ...
        info_dict['name'] = _name_info(sym)
        info_dict['prompt'] = _prompt_info(sym)
        info_dict['type'] = "Type: {}".format(TYPE_TO_STR[sym.type])
        info_dict['value'] = _value_info(sym)
        info_dict['help'] = _help_info(sym)
        info_dict['direct_dep'] = _direct_dep_info(sym)
        info_dict['defaults'] = _defaults_info(sym)
        info_dict['select_imply'] = _select_imply_info(sym)
        info_dict['kconfig_def'] = _kconfig_def_info(sym)

    elif isinstance(node.item, Choice):
        choice = node.item

        info_dict['name'] = _name_info(choice)
        info_dict['prompt'] = _prompt_info(choice)
        info_dict['type'] = "Type: {}".format(TYPE_TO_STR[choice.type])
        info_dict['mode'] = 'Mode: {}'.format(choice.str_value)
        info_dict['help'] = _help_info(choice)
        info_dict['choice_syms'] = _choice_syms_info(choice)
        info_dict['direct_dep'] = _direct_dep_info(choice)
        info_dict['defaults'] = _defaults_info(choice)
        info_dict['kconfig_def'] = _kconfig_def_info(choice)

    else:
        info_dict['kconfig_def'] = _kconfig_def_info(node)  # node.item in (MENU, COMMENT)

    return info_dict


# print a menu info
def print_menu_info(message):
    print("\n\nMQY INFO " + message)
    for info in _shown:
        print(_info_dict(info))
        # print(_info_str(info).split("\n"))


# refresh menu into _shown
def get_now_info():
    global _shown
    global _cur_menu
...
# go sub menu and refresh _shown
def go_sub(index):
    global _shown
    global _cur_menu
    shown_sub = _shown_nodes(_shown[index])
    _cur_menu = _shown[index]
    _shown = shown_sub


# go parent menu and refresh _shown
def go_parent():
    global _shown
    global _cur_menu
...
# set value on a index 
def set_a_value(index, value):
    global _shown
    global _sel_node_i
    global _cur_menu
    _sel_node_i = index
...
# quit and save configure to .config
def quit_and_save():
...
def _menuconfig(stdscr):
    ...
    _stdscr = stdscr
    logging.info("MQY stdscr" + str(_stdscr))
    _init()
    ...
    while True:
        ...
        if c == curses.KEY_RESIZE:
            ...
            sel_node = _shown[_sel_node_i]
            for menu_info in _shown:
                logging.info("MQY menu info: " + str(menu_info))
            # logging.info("MQY now press enter " + str(_sel_node_i))
            # logging.info("MQY aa" + str(sel_node))
            # logging.info("MQY bb" + str(_shown))
...
# 配置日志输出到文件
log_file_path = '/home/zlaa123456/mqy_log.txt'
...
def _include_path_info(node):
    ...
    include_path_info = "Included via {}\n".format(
        " -> ".join("{}:{}".format(filename, linenr)
                    for filename, linenr in node.include_path))

    # 输出日志信息
    logging.info(include_path_info)
...
def route_go_sub():
    ...
    if index is not None:
        ...
        return "Error: 'index' parameter is missing."


@app.route("/get_now_menu")
def route_get_now_menu():
    get_now_info()
    msg = ""
    for info in _shown:
        msg += "\n\n" + str(_info_dict(info))
...
def route_set_value():
    ...
    set_a_value(int(index), int(value))
    msg = ""
    for info in _shown:
        msg += "\n\n" + str(_info_dict(info))
    return msg

@app.route("/save_to_config")
def route_save_to_config():
    quit_and_save()
    try:
        with open(_conf_filename, 'r') as file:
            content = file.read()
...
if __name__ == "__main__":
    ...
    for key, value in environment_variables.items():
        logging.info(f'haha env: {key}: {value}')
    logging.info(f'hahahahaha:{sys.argv}')
    # print("222 MQY menu tree: " + str(_shown_nodes(standard_kconfig(__doc__))))
    _main() # init all global value needed
