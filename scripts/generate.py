...
def generate_build():
    ...
    with open(KRAFTCONFIG, "r", encoding="utf8") as stream:
        ...
        if 'targets' not in data.keys():
            print("No targets in {KRAFTCONFIG}.")
            return
...
""")
    ...
    with open(out_file, "w", encoding="utf8") as stream:
        ...
        if "rootfs" in run.keys():
            stream.write(f"\nrootfs=\"{run['rootfs']}\"\n")
            stream.write("test -d \"$rootfs\" || mkdir \"$rootfs\"\n")
        # Unpack filesystem, if present.
        if 'packed_rootfs' in run.keys():
            if 'rootfs' not in run.keys():
                print("rootfs absent. Aborting")
                return
            stream.write(f"tar xf \"{run['packed_rootfs']}\" -C \"$rootfs\"\n")
...
def generate_run_kraft(run, name, arch, plat, fs):
    ...
    if 'command' in run.keys():
        ...
        header = TEMPLATE_RUN_KRAFT_HEADER.format("")
    with open(out_file, "w", encoding="utf8") as stream:
        stream.write(header)
        if "rootfs" in run.keys():
        ...
        if arch == 'arm64':
            stream.write("    -W \\\n")
        stream.write(f"    --memory {run['memory']}M \\\n")
